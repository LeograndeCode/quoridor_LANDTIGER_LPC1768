...
void CAN_Init (void) {
  ...
  CAN_setup(CAN_CONTROLLER_FOR_GAME);
	
	 /* Enable reception for the necessary message ids */
	CAN_wrFilter (CAN_CONTROLLER_FOR_GAME, CAN_HANDSHAKE_ID, STANDARD_FORMAT);					
	CAN_wrFilter (CAN_CONTROLLER_FOR_GAME, CAN_HANDSHAKE_ACK_ID, STANDARD_FORMAT);
	CAN_wrFilter (CAN_CONTROLLER_FOR_GAME, 1, STANDARD_FORMAT);
	CAN_wrFilter (CAN_CONTROLLER_FOR_GAME, 2, STANDARD_FORMAT);
	
	
  CAN_start (CAN_CONTROLLER_FOR_GAME);                                  /* start CAN Controller  */
	
  CAN_waitReady (CAN_CONTROLLER_FOR_GAME);                              /* wait til tx mbx is empty */                             /* wait til tx mbx is empty */
